...
from WYFS1.functionHO import Fun
import random
random.seed(42)
...
def jfs(feature, label, opts):
    ...
    for i in range(N):
        fit[i,0] = Fun(feature, label, X[i,:], opts)
        if fit[i,0] < fitG:
            Xgb[0,:] = X[i,:]
            fitG     = fit[i,0]
    ...
    while t < max_iter:
        ...
        for i in range(2 * Nc):
            Fnew[i,0] = Fun(feature, label, Xnew[i,:], opts)
            if Fnew[i,0] < fitG:
                Xgb[0,:] = Xnew[i,:]
                fitG     = Fnew[i,0]
